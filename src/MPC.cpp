...
// Sets the timestep length and duration
size_t N = 10;
double dt = 0.1;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t i = 0; i < N - 1; i++) {
      ...
      fg[0] += CppAD::pow(vars[v_start], 2) * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += 10 * CppAD::pow(vars[a_start + i], 2);
    }

    for (size_t i = 0; i < N - 2; i++) {
      fg[0] += 100 * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 200 * CppAD::pow(vars[v_start], 2) * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
